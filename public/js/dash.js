...
var model = (function () {
  ...
  class Entry {
    constructor(id, text, mood, timeStamp) {
      this.id = id;
      this.text = text;
  ...
  function calculateIndMoodPerc(mood) {
    var totalMoodCount =
      data.moodCounter.happy +
      data.moodCounter.sad +
      data.moodCounter.neutral +
      data.moodCounter.unsure;

    // data.moodPerc[mood] = Math.floor((data.moodCounter[mood] / totalMoodCount) * 100 - (((data.moodCounter[mood] / totalMoodCount) * 100) * .02));

    data.moodPerc[mood] = Math.floor(
      (data.moodCounter[mood] / totalMoodCount) * 100
    );
  }
  ...
  return {
    ...
    displayEntries: function (entryList) {
      ...
      entryList.forEach(function (entry) {
        ...
        domEl.entriesDOM.insertBefore(
          entryItemDOM,
          domEl.entriesDOM.firstChild
        );
